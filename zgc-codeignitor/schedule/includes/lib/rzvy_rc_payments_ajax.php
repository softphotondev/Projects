...
if(isset($_REQUEST['refresh_rc_payments'])){
	...
	if(mysqli_num_rows($all_payments)>0){
		...
		while($payment = mysqli_fetch_assoc($all_payments)){
			...
			array_push($payment_arr, $rzvy_currency_symbol.$payment['sub_total']);
			array_push($payment_arr, $rzvy_currency_symbol.$payment['discount']);
			array_push($payment_arr, $rzvy_currency_symbol.$payment['refer_discount']);
			array_push($payment_arr, $rzvy_currency_symbol.$payment['tax']);
			array_push($payment_arr, $rzvy_currency_symbol.$payment['net_total']);
			array_push($payment_arr, strtoupper($payment['fd_key']));
			array_push($payment_arr, $rzvy_currency_symbol.$payment['fd_amount']);
